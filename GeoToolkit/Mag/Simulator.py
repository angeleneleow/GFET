...
from GeoToolkit.graphics import graphics, colors
...
def units():
    ...
    return {
        ...
        "derivativeY": "[nT/m]",
        "firstVertical": "[nT/m]",
        "totalHorizontal": "[nT/m]",
        "tiltAngle": "[rad]",
        "analyticSignal": "[nT/m]",
        "TDXderivative": "[Degree]",
        "RTP": "[nT]",
        "TMI": "[nT]",
    }
...
def ViewMagSurveyWidget(survey, shapeFile=None):
    def MagSurvey2D(East, North, Azimuth, Length, Sampling):
        ...
        plotProfile2D(xLoc, yLoc, data, a, b, Sampling, fig=fig, ax=ax2, ylabel="nT")
    ...
    out = widgets.interactive(
        ...
        Azimuth=widgets.FloatSlider(
            min=0, max=180, step=5, value=90, continuous_update=False
        ),
        Length=widgets.FloatSlider(
            min=20, max=diag, step=20, value=diag / 2.0, continuous_update=False
        ),
        Sampling=widgets.BoundedFloatText(
            min=10, max=1000, step=5, value=100, continuous_update=False
        )
...
def plotMagSurvey2D(
    ...
    ax.text(xLine[-1], yLine[-1], "A'", fontsize=16, color="k", ha="left")
    ax.grid(True)
...
def ViewPrism(survey):
    ...
    out = widgets.interactive(
        ...
        View_azm=widgets.FloatSlider(
            min=0, max=360, step=1, value=0, continuous_update=False
        ),
...
def plotObj3D(
    ...
    for prism, color in zip(prisms, colors):
        ...
        xyz = MathUtils.rotate(
            block_xyz.T, np.r_[prism.xc, prism.yc, prism.zc], pinc, pdec
        )
        ...
        axs.add_collection3d(
            Poly3DCollection([list(zip(xyz[:4, 0], xyz[:4, 1], xyz[:4, 2]))])
        )

        # Face 2
        axs.add_collection3d(
            Poly3DCollection(
                [list(zip(xyz[4:, 0], xyz[4:, 1], xyz[4:, 2]))], facecolors=color
            )
        )
...
def fitline(prism, survey):
    def profiledata(Binc, Bdec, Bigrf, depth, susc, comp, irt, Q, rinc, rdec, update):
        ...
        return plotProfile2D(
            xyzLoc, [survey2D.dobs, dpred], a, b, 10, dType="2D", ylabel="nT"
        )
    ...
    Q = widgets.interactive(
        ...
        depth=widgets.FloatSlider(min=0.0, max=2.0, step=0.05, value=0.5),
...
class MidPointNorm(Normalize):
    ...
    def __call__(self, value, clip=None):
        ...
        if self.midpoint is None:
            self.midpoint = (self.vmin + self.vmax) / 2.0
        ...
        if not (vmin < midpoint < vmax):
            ...
            resdat[resdat > 0] /= abs(
                vmax - midpoint
            )  # resdat[resdat > 0]= resdat[resdat > 0] / abs(vmax - midpoint)
...
def plotDataHillside(
    ...
    if fill:
        ...
        if equalizeHist == "HistEqualized":
            ...
            cdf, bins = exposure.cumulative_distribution(
                subGrid[(subGrid < vmax) * (subGrid > vmin)].flatten(), nbins=256
            )
        ...
        if np.all([alpha != 1, alphaHS != 0]):
            ...
            axs.imshow(
                ls.hillshade(d_grid, vert_exag=ve, dx=resolution, dy=resolution),
                cmap="gray_r",
                alpha=alphaHS,
                extent=extent,
                origin="lower",
    ...
    if contours is not None:
        ...
        CS = axs.contour(
            X, Y, d_grid, len(contours), levels=contours, colors="k", linewidths=0.5
        )

        # plt.clabel(CS, inline=1, fontsize=5, fmt='%i')

    if filledContour is not None:
        CSfilled = axs.contour(
            X, Y, d_grid, 1, levels=[filledContour], colors="r", linewidths=5.0
        )
    ...
    axs.set_aspect("auto")
    return X, Y, d_grid, im, CS, CSfilled
...
def plotData2D(
    ...
    if d is not None:
        ...
        if contours > 0:
            clevels = np.round(np.linspace(vmin, vmax, contours) * 1e-1) * 1e1
            ...
            if ~np.any(clevels == 0):
                clevels = np.sort(np.r_[clevels, 0])
            CS = axs.contour(
                X, Y, d_grid, contours, levels=clevels, colors="k", linewidths=0.5
            )
            # plt.clabel(CS, inline=1, fontsize=5, fmt='%i')

        if filledContour is not None:
            CSfilled = axs.contourf(
                X, Y, d_grid, contours, levels=clevels, colors="k", linewidths=0.5
            )
...
def plotProfile2D(
    ...
    distance = np.sqrt((xLine - a[0]) ** 2.0 + (yLine - a[1]) ** 2.0)
...
def dataHillsideWidget(
    ...
    def plotWidget(
        ...
        X, Y, data, im, CS, CSfilled = plotSave(
            gridObject,
            gridObject.values,
            scatterData,
            shapeFile,
    ...
    SunAngle = widgets.FloatSlider(
        min=0, max=90, step=5, value=15, continuous_update=False, description="SunAngle"
    )
    ...
    ColorMap = widgets.Dropdown(
        options=cmaps(), value="Spectral_r", description="ColorMap", disabled=False
    )
    ...
    filledContour = widgets.FloatSlider(
        min=vmin,
        max=vmax,
        step=5.0,
        value=filledContour,
    ...
    saveAs = widgets.Text(value=saveAs, description="File Name:", disabled=False)
...
def gridFiltersWidget(
    ...
    Contours = widgets.Text(
        value=Contours, description="Edge Detected zero contour boundary", disabled=False, continuous_update=False
    )
...
def gridTilt2Depth(
    ...
    def plotWidget(
        ...
        if SaveCSV:
            if len(polylines) > 0:
                ...
                np.savetxt(
                    CSVFileName,
                    np.c_[np.vstack(polylines)[:, 0:2], np.concatenate(attributes)],
                    fmt="%.2f",
                    delimiter=",",
                )
    ...
    GridFileName = widgets.Text(
        value=GridFileName, description="Save as:", disabled=False
    )
    ShapeFileName = widgets.Text(
        value=ShapeFileName, description="Shapefile name:", disabled=False
    )

    CSVFileName = widgets.Text(
        value=CSVFileName, description="CSV file name:", disabled=False
    )
    ContourColor = widgets.Dropdown(
        options=cmaps(), value="viridis", description="Depth Color", disabled=False
    )
...
def worldViewerWidget(worldFile, data, grid, z=0, shapeFile=None):
    def plotLocs(placeID):
        ...
        selection = int(
            np.r_[[ii for ii, s in enumerate(list(data.keys())) if placeID in s]]
        )
        ...
        locs = np.c_[
            Xloc.flatten(order="F"), Yloc.flatten(order="F"), Zloc.flatten(order="F")
        ]
        ...
        arrowAxs = fig.add_axes(
            [7, 7, pos.width * 0.5, pos.height * 0.5], projection="3d"
        )
        block_xyz = np.asarray(
            [
                [-0.2, -0.2, 0.2, 0.2, 0],
        ...
        # print xyz
        # Face 1
        arrowAxs.add_collection3d(
            Poly3DCollection(
                [list(zip(xyz[[1, 2, 4], 0], xyz[[1, 2, 4], 1], xyz[[1, 2, 4], 2]))],
                facecolors="w",
            )
        )
        ...
        arrowAxs.add_collection3d(
            Poly3DCollection(
                [list(zip(xyz[[0, 1, 4], 0], xyz[[0, 1, 4], 1], xyz[[0, 1, 4], 2]))],
                facecolors="k",
            )
        )
        ...
        arrowAxs.add_collection3d(
            Poly3DCollection(
                [list(zip(xyz[[2, 3, 4], 0], xyz[[2, 3, 4], 1], xyz[[2, 3, 4], 2]))],
                facecolors="w",
            )
        )
        ...
        arrowAxs.add_collection3d(
            Poly3DCollection(
                [list(zip(xyz[[0, 3, 4], 0], xyz[[0, 3, 4], 1], xyz[[0, 3, 4], 2]))],
                facecolors="k",
            )
        )
        ...
        arrowAxs.add_collection3d(
            Poly3DCollection(
                [list(zip(xyz[:4, 0], xyz[:4, 1], xyz[:4, 2]))], facecolors="r"
            )
        )
...
def dataGriddingWidget(
    ...
    def plotWidget(
        Resolution, MaxDistance, Method, ColorMap, EPSGcode, GetIncDec, saveAs, SaveGrid
    ):
        ...
        if Method == "minimumCurvature":
            ...
            dists, _ = tree.query(np.c_[X.flatten(order="F"), Y.flatten(order="F")])
        ...
        gridObject.nx, gridObject.ny = (
            gridObject.values.shape[1],
            gridObject.values.shape[0],
        )

        gridObject.dx = (X.max() - X.min()) / (gridObject.values.shape[1] - 1)
        gridObject.dy = (Y.max() - Y.min()) / (gridObject.values.shape[0] - 1)

        gridObject.x0, gridObject.y0 = (
            X.min() - gridObject.dx / 2.0,
            Y.min() - gridObject.dy / 2.0,
        )

        gridObject.limits = np.r_[
            gridObject.x0,
            gridObject.x0 + gridObject.nx * gridObject.dx,
            gridObject.y0,
            gridObject.y0 + gridObject.ny * gridObject.dy,
        ]
    ...
    def fetchURL(_):
        if GetIncDec.value:
            ...
            x, y, z = np.mean(xLoc), np.mean(yLoc), 0.0
    ...
    Resolution = widgets.FloatText(value=25, description="Grid (m):", disabled=False)

    MaxDistance = widgets.FloatText(
        value=200, description="Dist Max (m):", disabled=False
    )

    Method = widgets.Dropdown(
        options=["nearest", "linear", "cubic", "minimumCurvature"],
        value=Method,
        description="Method",
        disabled=False,
    )
...
def dataGridGeoref(
    ...
    def plotWidget(
        ColorMap, EPSGcode, GetIncDec, inc, dec, ndv, units, applyRTP, saveAs, SaveGrid
    ):
    ...
    def fetchURL(_):
        if GetIncDec.value:
            ...
            x, y, z = np.mean(gridObject.hx), np.mean(gridObject.hy), 0.0
            # input SpatialReference
            inSpatialRef = osr.SpatialReference()
            inSpatialRef.ImportFromEPSG(int(EPSGcode.value))
    ...
    if np.any(gridObject.values > 1e10):

        ndv = np.unique(gridObject.values[gridObject.values > 1e10])
    ...
    ndv = widgets.FloatText(value=ndv, description="No-data-Value", disabled=False)
...
def setDataExtentWidget(
    ...
    def dataSelector(East, North, Width, Height, EPSGcode, units, saveAs, SaveGrid):

        lims = np.r_[
            East - Width / 2, East + Width / 2, North - Height / 2, North + Height / 2
        ]

        fig, axs = plt.figure(figsize=(12, 6)), plt.subplot(1, 2, 1)
        plotData2D(
            xLoc, yLoc, survey.values, marker=False, fig=fig, ax=axs, colorbar=False
        )
        ...
        dataSub.x0, dataSub.y0 = (
            xLoc[indx].min() - survey.dx / 2,
            yLoc[indy].min() - survey.dy / 2,
        )
        dataSub.EPSGcode = survey.EPSGcode

        dataSub.limits = np.r_[
            dataSub.x0,
            dataSub.x0 + dataSub.nx * dataSub.dx,
            dataSub.y0,
            dataSub.y0 + dataSub.ny * dataSub.dy,
        ]
...
def plotSave(
    ...
    # Add shading
    X, Y, data, im, CS, CSfilled = plotDataHillside(
        gridObject.hx,
        gridObject.hy,
        data,
        axs=axs,
    ...
    if SaveGrid:
        ...
        if (scatterData is not None) and scatterData["colorbar"]:
            pos = axs.get_position()
            cbarax = fig.add_axes(
                [pos.x0 + 0.875, pos.y0 + 0.225, pos.width * 0.025, pos.height * 0.4]
            )
            norm = mpl.colors.Normalize(
                vmin=scatterData["clim"][0], vmax=scatterData["clim"][1]
            )
            cmap = mpl.cm.get_cmap(name=scatterData["cmap"])
            cb = mpl.colorbar.ColorbarBase(
                cbarax, cmap=cmap, norm=norm, orientation="vertical"
            )
            cb.set_label("Depth (m)", size=12)

    return X, Y, data, im, CS, CSfilled
